...
def load_resources():
    ...
    try:
        ...
        faiss_index = faiss.read_index(os.path.join(BASE_DIR, "travel_index.faiss"))
    except Exception as e:
...
CATEGORY_MAP = {
    "beach": [
        "Goa", "Gokarna", "Varkala", "Kovalam",
        "Puri", "Digha", "Andaman & Nicobar"
    ],

    "temple": [
        "Tirupati", "Kedarnath", "Badrinath",
        "Somnath", "Meenakshi Temple",
        "Golden Temple", "Jagannath Temple"
    ],

    "mountain": [
        "Ladakh", "Auli", "Manali",
        "Spiti Valley", "Darjeeling",
        "Gangtok", "Nainital"
    ],
...
def generate_plan(travel_facts, days, budget):
    ...
    prompt = f"""
You are a professional India travel planner.
...
CATEGORY_KEYWORDS = {
    "beach": ["beach", "sea", "coast", "waterfront"],
    "temple": ["temple", "mandir", "shrine"],
    "mountain": ["mountain", "hill", "peak"],
    "city": ["city", "metro", "urban"],
    "adventure": ["trek", "hike", "rafting", "adventure"],
    "nature": ["forest", "river", "lake", "waterfall"]
}
...
def fetch_images(place, user_query, count=3):
    """
    Returns a list of image URLs using Pexels API
    """

    category = detect_category(user_query)
    search_term = f"{category} {place} India travel"
    ...
    try:
        ...
        r = requests.get(PEXELS_URL, headers=headers, params=params, timeout=5)
        data = r.json()

        if "photos" in data and len(data["photos"]) > 0:
            return [img["src"]["medium"] for img in data["photos"][:count]]

    except Exception as e:
        print("Pexels error:", e)

    # fallback image
    return [f"https://source.unsplash.com/600x400/?{place},travel"] * count
...
def ask():
    ...
    if category:
        boosted_places = ", ".join(CATEGORY_MAP[category])
        user_query = user_query + f" Include popular {category} destinations like {boosted_places}."

    budget = data.get("budget", "").strip()
    if budget:
        budget_instruction = f"User budget is strictly ₹{budget}. Do not exceed it."
    else:
        budget_instruction = "No fixed budget — estimate affordable India travel."
    ...
    try:
        ...
        response["images"] = fetch_images(destination, user_query, count=3)
